...
class DataIngestion:
    ...
    def download_files(self, n_day_interval_url: int = None):
        try:
            ...
            for index in range(1,len(required_interval)):
                ...
                logger.debug(f'Generating data url between {from_date} and {to_date}')
                datasource_url = self.data_ingestion_config.datasource_url
                url = datasource_url.replace("<fromdate>", from_date).replace("<todate>", to_date)
    ...
    def retry_download_data(self, data, download_url: DownloadUrl):
        try:
            ...
            # to handle the throatling requestation and can be solve if we wait for some seconds
    ...
    def download_data(self,download_url:DownloadUrl):
        try:
            logger.info(f"Starting the download operation: {download_url}")
            ...
            try:
                ...
                #saving the download into the local folde
